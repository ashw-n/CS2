...
ASSIGNMENT_NAME = "Senior Design Team Builder"
NUMBER_OF_TEST_CASES = 5
FILE_NAME = "TeamBuilder"
...
"""
def bit_bunny_message(student_name: str,
                      name_wrap_threshold: int = 22,
                      wrap_width: int = 28) -> str:
...
def result():
    ...
    for tc in range(1, NUMBER_OF_TEST_CASES + 1):
        ...
        instructor_solution = "tc" + str(tc) + ".out"
        student_solution = "tc" + str(tc) + "student.out"
...
def compile_run_file():
    ...
    # Capture compile errors or warnings.
    if len(str(proc.stderr)) > 0:

        print(
            "Uh oh... Your code had compile warning/error messages! "
            "Try to fix them or else points are deducted."
        )
    ...
    for tc in range(1, NUMBER_OF_TEST_CASES + 1):
        ...
        f = open("tc" + str(tc) + "student.out", "w")
...
def setup_checker():
    ...
    print(
        "Before testing your solution, we need to check to see if all "
        "files are properly placed in the same directory."
    )
    ...
    for tc in range(1, NUMBER_OF_TEST_CASES + 1):
        ...
        if not os.path.exists(
            ...
            raise FileNotFoundError(
                "Missing tc"
                + str(tc)
                + ".out in the directory. Script Exiting!"
            )

    # Check student source file.
    if not os.path.exists(cwd + "/" + JAVA_FILE):

        raise FileNotFoundError(
            "You are missing your Java source!! "
            "Please put in your Java file. Script Exiting!"
        )
    ...
    if not os.path.exists(cwd + "/" + JAVA_DRIVER_FILE):
        ...
        raise FileNotFoundError(
            "You are missing your Java Driver source!! "
            "Please put in your Java Driver file. Script Exiting!"
        )

    print(
        "We are done checking to make sure necessary files were provided."
    )
